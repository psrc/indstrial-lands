...
UPDATE Sandbox.Mike.ili20231023 SET Shape = Shape.MakeValid();
EXEC sp_rename 'Mike.ili20231023.ind_catego', 'ind_type', 'COLUMN'

CREATE SPATIAL INDEX ili20231023_sidx  ON Sandbox.Mike.ili20231023(Shape) USING GEOMETRY_AUTO_GRID
...
FROM employment.workplace_location_dims AS wl JOIN Sandbox.Mike.ili20231023 AS ili ON wl.Shape.STIntersects(ili.Shape)=1
...
WITH cte AS (
SELECT wf.data_year, 
        CASE WHEN ili.ind_type <>'Limited Industrial' AND ili.ind_type IS NOT NULL THEN 'Industrial' 
        WHEN ili.ind_type='Limited Industrial' THEN 'Limited Industrial' ELSE 'Not Industrial' END AS Industrial_Land_Status, 
        i.manuf_industrial_group,
          CASE WHEN SUM(wf.jobcount_covered) < 0.4 THEN '0' 
               WHEN SUM(i.private_sector) = 0 THEN CAST(CAST(ROUND(SUM(wf.jobcount_covered),0) AS int) AS nvarchar)
               WHEN SUM(i.private_sector) IN(1,2) 
        ...
        GROUP BY wf.data_year, 
        CASE WHEN ili.ind_type <>'Limited Industrial' AND ili.ind_type IS NOT NULL THEN 'Industrial' 
        WHEN ili.ind_type='Limited Industrial' THEN 'Limited Industrial' ELSE 'Not Industrial' END, 
                 i.manuf_industrial_group
        HAVING wf.data_year IS NOT NULL),
 cte2 AS (SELECT * FROM cte PIVOT(max(emp_covered) FOR data_year IN([2005],[2010],[2015],[2020],[2022])) AS p)
SELECT * from cte2 ORDER BY Industrial_Land_Status, manuf_industrial_group;
